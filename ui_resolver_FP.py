...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.label_2.setGeometry(QRect(40, 20, 511, 31))
...
"font: 900 20pt \"Segoe UI Black\";")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.label_2.setText(QCoreApplication.translate("MainWindow", u"Ecuaci\u00f3n a ser resuelta por m\u00e9todo de falsa posici\u00f3n:", None))
...
class MainWindow(QMainWindow):
    ...
    def solucion(self):
        ...
        while not error_achieved:
            ...
            print(f"a*f_b: {a*f_b}, b*f_a: {b*f_a}, a*f_b - b*f_a: {a*f_b - b*f_a}, f_a - f_b: {f_a - f_b}")
            print(f"xr: {xr}")
...
Luego, se calcula el punto medio entre estos dos puntos con la formula (a*f(b)-b*f(a))/(f(b)-f(a)) y se ubica en la columna Xr. En este caso es {xr}.\n
...
Ya que esta es la primera iteración, no se coloca nada en la columna error.
            ...
            if iteration > 0:
                error = (abs((xr - pasos[iteration - 1].xr)/xr))*100
...
Luego, se calcula de nuevo el punto medio entre estos dos puntos con la formula (a*f(b)-b*f(a))/(f(b)-f(a)) y se ubica en la columna xr. En este caso es {xr}.\n
Ahora, se calcula el valor de la ecuacion en el punto a, b y xr y se ubican en las columnas f(a), f(b) y f(xr) respectivamente.\n
...
Se calcula el error con la formula |(xr-xr_anterior)/xr|, en este caso es {(abs((b-a)/2))*100} y se ubica en la columna error. En este caso es {error}
                """
...
if __name__ == "__main__":
    ...
    ecuacion_prueba = Eq(x**2 - 4-2, 0)
    window = MainWindow(ecuacion_prueba, "x^2-4-2=0")
    window.show()
    sys.exit(app.exec())